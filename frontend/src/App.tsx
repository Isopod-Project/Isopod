                     ...
                     <div className="flex gap-2">
                        ...
                        {addStep === 1 ? (
                           <button
                              ...
                              className={`flex items-center gap-2 px-8 py-2 rounded font-bold text-sm shadow-xl transition-all ${!newName || (addTab !== 'custom' && addTab !== 'import' && !selectedModpack) ? 'bg-neutral-800 text-neutral-600 cursor-not-allowed border border-[#333]' : 'bg-[#3E8ED0] hover:bg-[#2B6A9E] text-white shadow-[#3E8ED0]/10'
                                 }`}
                           >
                                 ...
                                 >
                                    {(() => {
                                       ...
                                       orphaned.forEach((m, i) => {
                                          renderedLines.push(
                                             <div key={`orph-${i}`} className="opacity-70 border-l-2 border-amber-500/20 pl-2 mb-2 italic text-[10px]">
                                                {m.content}
                                             </div>
                                          );
                                       });

                                       const linesToRender = isVerbose ? sLines : sLines.filter(line => {
                                          const trimmed = line.trim();
                                          if (manual.some(m => m.anchor && m.anchor.trim() === trimmed)) return true;
                                       ...
                                       linesToRender.forEach((line, idx) => {
                                          ...
                                          matches.forEach((m, midx) => {
                                             renderedLines.push(
                                                <div key={`m-${idx}-${midx}`} className="py-1 border-l-2 border-emerald-500/30 pl-2 my-1 bg-emerald-500/5 transition-all animate-in fade-in slide-in-from-left-1">
                                                   {m.content}
                                                </div>
                                             );
                                          });
